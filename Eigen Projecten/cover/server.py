...
englishstrings = ["card is ",  # 0
                  ...
                  " has ",
                  "is ",
                  "Game over: these are the results"]  # 54
...
dutchstrings = ["kaart is ",  # 0
                ...
                "\'s Kaarten: ",
                " Score = ",
                "trek kaart, pak weggegooide kaart, cover of dubbele kaart",
                "trek kaart",
                "pak weggegooide kaart",
                "weggegooide",
                "cover",
                "dubbele kaart",  # 10
                "een kaart getrokken ",
                "je trok ",
                "je kan een van je eigen kaarten bekijken, wil je dat?",
                "ja",
                "Welke kaart wil je bekijken",
                "bekeek eigen kaart nummer ",
                "je kan een andermans kaart bekijken, wil je dat?",
                "heeft bekeken ",
                "\'s kaart nummer ",
                "Je kan een kaart verwisselen zonder te kijken, wil je dat?",  # 20
                "eigen kaart nummer ",
                "ander zijn kaart nummer ",
                "heeft gewisseld",
                "met eigen kaart nummer ",
                "Je kan een kaart verwisselen met kijken, wil je dat?",
                "Wil je wisselen?",
                "Met welke kaart nummer ",
                "en heeft gewisseld met eigen kaart nummer ",
                "Wat wil je doen? [gooi weg, vervang]",
                "gooi weg",  # 30
                "vervang",
                "heeft getrokken kaart weggegooid",
                "Met welke kaart wil je vervangen",
                "heeft getrokken kaart vervangen met kaart nummer ",
                "ongeldige actie",
                "heeft weggegooide kaart getrokken",
                "Met welke kaart wil je vervangen",
                "Heeft weggegooide kaart vervangen met eigen kaart nummer ",
                "heeft gecovered",
                "gooit dubbele kaart van ",  # 40
                "Welke kaart nummer is een dubbele",
                "Welke van je eigen kaarten wil je vervangen",
                "en heeft gewisseld met eigen kaart nummer ",
                "Kaarten zijn niet dubbel",
                "ongeldige kaart nummer ",
                "quit",
                "ongeldige opdracht, probeer opnieuw",
                "Welke speler?",
                "speler niet gevonden",
                "speler naam?",  # 50
                "heeft Spelers naam veranderd in ",
                " heeft ",
                "is ",
                "Spel voorbij, dit zijn de resultaten"]  # 54
...
class player:
    ...
    def PlayTurn(self, players):
        while True:
            ...
            if action == languagestrings[self.language][6]:
                ...
                if self.PlayCard.Power == 1:
                    ...
                    if action == languagestrings[self.language][14]:
                        ...
                        # Which card do you want to look at
                        send_msg('request: ' + languagestrings[self.language][15], self, self.ID)
                        cardnumber = int(tostr(self.connection.recv(1024)))
                    ...
                    if action == languagestrings[self.language][14]:
                        ...
                        opponentcardnumber = int(tostr(self.connection.recv(1024)))
                        # Own card number?
                        send_msg('request: ' + languagestrings[self.language][21], self, self.ID)
                        owncardnumber = int(tostr(self.connection.recv(1024)))
                        ...
                        self.sendaction(players, [23, str(players[playernumber].ID), 19, str(opponentcardnumber), 24,
                                                  str(owncardnumber)])
                        break
            ...
            # vanaf hier omzetten naar multiplayer duurt wel lang
    ...
    while True:  # main game loop on player thread
        ...
        if player.startgame and player.Turn:
            ...
            # Else, output the game state and have the player do a turn
                ...
                player.outputgamestate(players, TrashDeck, 0)
        ...
        # Send the end state of the game when a game has ended, showing all the cards
            ...
            send_msg(languagestrings[player.language][54], player, player.ID)
            for x in players:
                for y in x.cards:
                    y.HasSeen(player.ID)
            player.outputgamestate(players, TrashDeck, 1)
...
def send_msg(message, player, ID):
    ...
    while True:
        ...
        returnmessage = tostr(connection.recv(1024))
        if returnmessage == message:
            player.connection.sendall(bytes('ok', 'utf-8'))
            player.status = 'send ok'
            print('message arrived at ' + str(ID) + ' ok')
            returnmessage = tostr(connection.recv(1024))
            if returnmessage == 'ok':
                break
        else:
            print('message not returned correctly from ' + str(ID))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        players.append(player(connection, len(players)))
        players[-1].reset()
