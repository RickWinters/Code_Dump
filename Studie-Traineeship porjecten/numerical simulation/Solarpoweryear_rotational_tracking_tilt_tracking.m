...
minutes = 1:1/60:24;                                                        % Array of minutes in a day
tic                                                                         % Start timer
mintilt = 0;                                                                % Minimun tilt of solar panel
maxtilt = 90;                                                               % Maximum tilt of solar panel
yearlyplot = 1;                                                             
dailyplot = 0;

maxpower = [];                                                              % Create an empty array of max power production per day
totaldailyenergy = [];                                                      % Create an empty array of total energy production per day
Totalenergy = 0;                                                            % Set Totalenergy to 0
for i = 2:366                                                               % Loop over all days of the year
    strcat('current: ',num2str(i-1),'/365')                                 % Update user over command window
    ...
    invangles = 90 - minuteangles;
    SPangles = invangles;                                                   % Inverse the angles so that 0 degree tilt is the panel on its side, facing the horizon, which coincides with 0 degree elevation of the sun
    SPangles(SPangles >= maxtilt) = maxtilt;                                % Limit the tilt of the panel to the maxtilt
    SPangles(SPangles <= mintilt) = mintilt;                                % Limit the tilt of the panel to the mintilt
    angledif = invangles - SPangles;                                        % Calculate the difference of the SPnormal and SUN, there might be a difference because of tilt limitations
    angledif(ind) = 90;                                                     % Set the diff to 90 degrees when the sun hasn't risen yet, because cosd(90) = 0,
    ...
    totaldailyenergy = [totaldailyenergy, sum(output)];                     % Keep track of totaldaily energy
