...
minutes = 1:1/60:24;                                                        % Array of minutes troughout the day
SPangle = 0:1:90;                                                           % Array of tilt-angles of the solar panel
tic                                                                         % Start timer

yearlyplot = 0;

parfor u = 1:91                                                             % Parallel loop over all tilt angles, Multiple proceses get started where each process calculates this loop iindependently of the other
    Totalenergy(u) = 0;                                                     % Create an array of Totalenery
    maxpower = [];                                                          % Set maxpower to an empty array, this gets added to the cell-matrix
    Maxpower{u} = [];                                                       % Create a cell-matrix entry that is an empty array.  
    Totaldailyenergy{u} = [];                                               % Set the cell-matrix entry to an empty array                 
    totaldailyenergy = [];                                                  % Create an ampty array representing the total powergenerated for each day in the month.
