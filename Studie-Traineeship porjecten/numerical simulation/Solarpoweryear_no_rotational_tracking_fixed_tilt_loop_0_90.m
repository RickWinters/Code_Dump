...
load azimuth.mat                                                            % Load in the azimuthangles data
...
basepower = 75;                                                             % Watt
SPazimuth = 180;                                                            % Angle on a compass of the solar panel face
timestep = 1;                                                               % In minutes
minutes = 1:1/60:24;                                                        % Array of minutes throughout the day
SPangle = 0:1:90;                                                           % Array of tilt-angles of the solar panel
tic                                                                         % Starts timer
dailyplot = 0;                                         
yearlyplot = 1;

parfor u = 1:91                                                             % Parallel loop over all tilt angles, Multiple processes get started where each process calculates this loop independently of the others and at the same time, speeding up the process
    Totalenergy(u) = 0;                                                     % Create an array of Totalenergy
    Maxpower{u} = [];                                                       % Create an cell-matrix of MaxPower
    maxpower = [];                                                          % Set maxpower to an empty array, this array gets added to the cell matrix
    totaldailyenergy = [];                                                  % Create an empty array representing the total power generated for each day in the month
    Totaldailyenergy{u} = [];                                               % Set the cell-matrix entry to an empty array
    for i = 2:366                                                           % Loop over all days in the year
        strcat('current: ',num2str(SPangle(u)),' deg    days',num2str(i),'/365') %Update the user through command window
        ...
        SPangles(ind) = 0;
        elevation_efficiencies = 1 - cosd(SPangles);                        % Calculate the efficiency of the panel regarding the angle of the sun on the panel
        
        %calculate efficiencies based on azimuth angle differenc
        azimuth_current = azimuth(i,:);                                     % Load in azimuth angles of the sun this day
        weights = azimuth_current > 0;                                      % Only take the azimuth of the sun into consideration where it has risen to fit it for every minute
        fitted = fit((1:1:24)',azimuth_current','d.*x.^3+a.*x.^2+b.*x+c','weight',weights); % Fit the azimuth with a cubic fit
        azimuth_fitted = fitted(minutes);                                   % Evaluate fit
        azdifference = abs(azimuth_fitted - SPazimuth);                     % Calculate difference in azimuthe angle
        azdifference(azdifference > 90) = 90;                               % If its bigger than 90 degrees, set it to 90 degrees
        azimuthefficiencies = 1-sind(azdifference);                         % Calculate efficiency in the same way elevation efficiency is calculated. as a first assumption this should do
        
        efficiencies = azimuthefficiencies .* elevation_efficiencies;       % Multiply elevation and azimuth together to get total efficiency
        ...
        totaldailyenergy = [totaldailyenergy, sum(output)];                 % Remember total daily output in array
        maxpower = [maxpower, max(power)];                                  % Remember the max power output of this day
        
        if dailyplot == 1                                                   % This plot is used for debugging purposes
    ...
    Maxpower{u} = maxpower;                                                 % Save max power output per day in cell-matrix, so that Matlab main process can acces it
    Totaldailyenergy{u} = totaldailyenergy;                                 % Save totaldailyenergy to cell-matrix, so that main process can acces it
end

ind = find(Totalenergy == max(Totalenergy));                                % Find the index of the maximum total energy, this is also the optimum angle
